...
stack-start:  ## Local Stack: Start Services
	...
	@echo "Now visit: http://mylibrary.localhost"
...
build-acceptance-servers: ## Build Acceptance Servers
	...
	@docker build backend -t GhitaB/mylibrary-backend:acceptance -f backend/Dockerfile.acceptance
	@echo "Build acceptance frontend"
	@docker build frontend -t GhitaB/mylibrary-frontend:acceptance -f frontend/Dockerfile

.PHONY: start-acceptance-servers
start-acceptance-servers: build-acceptance-servers ## Start Acceptance Servers
	@echo "Start acceptance backend"
	@docker run --rm -p 55001:55001 --name mylibrary-backend-acceptance -d GhitaB/mylibrary-backend:acceptance
	@echo "Start acceptance frontend"
	@docker run --rm -p 3000:3000 --name mylibrary-frontend-acceptance --link mylibrary-backend-acceptance:backend -e RAZZLE_API_PATH=http://localhost:55001/plone -e RAZZLE_INTERNAL_API_PATH=http://backend:55001/plone -d GhitaB/mylibrary-frontend:acceptance

.PHONY: stop-acceptance-servers
stop-acceptance-servers: ## Stop Acceptance Servers
	@echo "Stop acceptance containers"
	@docker stop mylibrary-frontend-acceptance
	@docker stop mylibrary-backend-acceptance
